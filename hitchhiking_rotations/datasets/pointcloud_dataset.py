...
class PointCloudDataset(Dataset):
    def __init__(self, mode, device):
        ...
        self.feature_input = torch.from_numpy(np.concatenate((pcd, rotated_pcd), axis=-1).transpose((0, 2, 1)).astype(np.float32)).to(self.device)
...
def create_dataset():
    ...
    class PointSampler(object):
        ...
        def __call__(self, mesh):
            ...
            sampled_faces = np.array(random.choices(faces,
                                            weights=areas,
                                            cum_weights=None,
                                            k=self.output_size))

            sampled_points = self.sample_point_vectorized(verts[sampled_faces[:,0]],
                                                        verts[sampled_faces[:,1]],
                                                        verts[sampled_faces[:,2]])
    ...
    if not(path_train_rotated.exists() and path_train_rots.exists() and path_test_rotated.exists() and path_test_rots.exists()):
