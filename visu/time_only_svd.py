...
for batch_size in batch_sizes:
    ...
    for j, batch in enumerate(test_dataloader):
        ...
        for trainer_name, pretty_name in zip(
            ["r9_svd_geodesic_distance", "r6_gso_geodesic_distance"],
            [str(s) for s in [RotRep.SVD, RotRep.GSO]],
        ):
...
out_p = os.path.join(experiment_folder, "rebutal_timing_only_svd.pdf")
plt.savefig(out_p)
save_pickle(timing_result, os.path.join(experiment_folder, f"seed_{s}_timing_only_svd.npy"))
