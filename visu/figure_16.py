...
def load():
    def get_joint_pos_rot_arr(joints, amc_path, frame, joint_names):
        ...
        out_arr = np.array([d[j].coordinate.flatten().tolist() + d[j].rot_in_parent.flatten().tolist() for j in joint_names])
    ...
    if not DS_SAMPLED.exists():
        ...
        datasets_s['sampled_frames'] = datasets_s['n_frames'].map(lambda n_frame: random.sample(range(n_frame), N_frames_sampled))
    ...
    if not SAMPLED_10K.exists():
        ...
        # Save to csv
        dataset_10k = pd.DataFrame({'path': paths, 'frame': frames, 'Subject': subjects, 'Activity': activities, 'asf_path': asf_paths})
        joint_names = list(parse_asf(dataset_10k['asf_path'][0]).keys())
        dic_of_joints = {asf:  parse_asf(asf) for asf in tqdm(dataset_10k['asf_path'].unique(), desc='Parsing asf files')}

        tqdm.pandas(desc='Computing joint pos and rot for each motion')
        dataset_10k['pos_rot'] = dataset_10k.progress_apply(lambda row: get_joint_pos_rot_arr(dic_of_joints[row.asf_path], row.path, row.frame, joint_names), axis=1)
    ...
    if not T_POSE_PATH.exists():
        ...
        t_pose_df = pd.DataFrame({'Subject': ['87'], 'Activity': ['02'], 'path': [tpose_amc_path], 'asf_path': [tpose_asf_path], 'pos_rot': [t_pose_pos_rot]})
...
def plot_distances():
    ds_rot_tpose_2_rot = Rotation.from_matrix(np.load(NUMPY_DS / 'ds_rot_tpose_2_rot.npy').reshape(-1 ,3, 3)) # N*Njoints, 3, 3  = 10k*31, 3, 3
